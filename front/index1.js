...
const ID_GENE_1     = conf.width * conf.height * 4
...
const SHADER = `
  ...
  fn main([[builtin(global_invocation_id)]] gid : vec3<u32>) {
    ...
    // Leave
    if (in_kind == ${ANT}u && in_direction != ${STAY}u) {
      out_kind = ${FREE}u;
      out_pheromon = 255u;
    }
    ...
    // Go right
    if (left_kind == ${ANT}u && left_direction == ${RIGHT}u) {
      out_kind = ${ANT}u;
      out_direction = left_direction;
    }
    // Go up
    if (down_kind == ${ANT}u && down_direction == ${UP}u) {
      out_kind = ${ANT}u;
      out_direction = down_direction;
    }
    // Go left
    if (right_kind == ${ANT}u && right_direction == ${LEFT}u) {
      out_kind = ${ANT}u;
      out_direction = right_direction;
    }
    // Go down
    if (up_kind == ${ANT}u && up_direction == ${DOWN}u) {
      out_kind = ${ANT}u;
      out_direction = up_direction;
    }

    // Source
    if (left_kind == ${ANTHILL}u && left_life == rythm_ && left_direction == ${RIGHT}u) {
      out_kind = ${ANT}u;
      out_direction = left_direction;
    }
    if (up_kind == ${ANTHILL}u && up_life == rythm_ && up_direction == ${DOWN}u) {
      out_kind = ${ANT}u;
      out_direction = ${DOWN}u;
    }
    if (right_kind == ${ANTHILL}u && right_life == rythm_ && right_direction == ${LEFT}u) {
      out_kind = ${ANT}u;
      out_direction = right_direction;
    }
    ...
    if (out_kind == ${ANT}u) {
      if (out_direction == ${STAY}u || r3 > 0.9) {
        out_direction = random_u32(r1, f32(gid.x), f32(gid.y), 0u, 4u);
      }
      ...
      if (i.n[pheromon(right(x))] > 2u && out_direction == ${RIGHT}u) {
        ...
        if (right_kind != ${FREE}u && right_kind != ${PLANT}u && out_direction == ${RIGHT}u) {
          out_direction = ${STAY}u;
        }
        if (up_kind != ${FREE}u && right_kind != ${PLANT}u && out_direction == ${UP}u) {
          out_direction = ${STAY}u;
        }
        if (i.n[kind(left(x))] != ${FREE}u && right_kind != ${PLANT}u && out_direction == ${LEFT}u ) {
          out_direction = ${STAY}u;
        }
        if (down_kind != ${FREE}u && right_kind != ${PLANT}u && out_direction == ${DOWN}u) {
          out_direction = ${STAY}u;
        }
    }
    ...
    if (r1 > 0.6) {
      if (up_kind == ${PLANT}u && out_kind == ${FREE}u) {
        out_kind = ${PLANT}u;
        out.n[gene_1(x)] = i.n[gene_1(up(x))] - 5u + random_u32(r1, r1, r2, 0u, 10u);
      }
      if (left_kind == ${PLANT}u && out_kind == ${FREE}u) {
        out_kind = ${PLANT}u;
        out.n[gene_1(x)] = i.n[left(gene_1(x))] - 5u + random_u32(r2, f32(gid.x), f32(gid.y), 0u, 10u);
      }
      if (right_kind == ${PLANT}u && out_kind == ${FREE}u) {
        out_kind = ${PLANT}u;
        out.n[gene_1(x)] = i.n[right(gene_1(x))] - 5u + random_u32(r3, f32(gid.x), f32(gid.y), 0u, 10u);
      }
      if (down_kind == ${PLANT}u && out_kind == ${FREE}u) {
        out_kind = ${PLANT}u;
        out.n[gene_1(x)] = i.n[gene_1(down(x))] - 5u + random_u32(r4, f32(gid.x), f32(gid.y), 0u, 10u);
      }
      out.n[gene_1(x)] = out.n[gene_1(x)] % 255u;
    }
    ...
    if (out_kind == ${PLANT}u) {
      canvas.n[pix_id] = 0u;
      canvas.n[pix_id+1u] = 255u;
      canvas.n[pix_id+2u] = i.n[gene_1(x)];
    } else {
